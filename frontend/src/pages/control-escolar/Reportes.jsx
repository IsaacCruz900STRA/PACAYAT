// src/pages/control-escolar/Reportes.jsx
...
export default function ControlEscolarReportes() {
  ...
  const seleccionarAlumno = (a) => {
    setAlumno(a); setQuery(a.nombre);
    setShowDrop(false); setResultados([]);
    setFiltro('Todos'); setVistaGlobal(false);
  };
  ...
  const reportesFiltrados = filtro==='Todos' ? reportesAlumno : reportesAlumno.filter(r=>r.tipo===filtro);

  const reportesGlobales = filtroTipo==='Todos' ? REPORTES_MOCK : REPORTES_MOCK.filter(r=>r.tipo===filtroTipo);
  const countPos = REPORTES_MOCK.filter(r=>r.tipo==='Positivo').length;
  const countNeg = REPORTES_MOCK.filter(r=>r.tipo==='Negativo').length;
  ...
  return (
    <div style={{ padding:'0 2rem 2rem' }}>
      <PageHeader title="Reportes" subtitle="Historial de conducta del alumnado" />
      ...
      <Card style={{ padding:'1.25rem', marginBottom:'1.5rem' }}>
          ...
          {showDrop && resultados.length>0 && (
            <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'#fff', border:'1.5px solid var(--border)', borderRadius:'var(--radius)', boxShadow:'var(--shadow-lg)', zIndex:100, overflow:'hidden' }}>
              {resultados.map(a=>{
                ...
                return (
                  <div key={a.id} onClick={()=>seleccionarAlumno(a)}
        ...
        {alumno && (
          <div style={{ marginTop:10, display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ fontSize:13, color:'var(--text-secondary)' }}>Historial de:</span>
            ...
            <button onClick={limpiar} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-secondary)', fontSize:13, fontFamily:'inherit' }}>
          ...
          <Card style={{ padding:'1rem 1.25rem', marginBottom:'1.25rem', display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            ...
            {[
              ...
              <button key={f.val} onClick={()=>setFiltroTipo(f.val)} style={{
                ...
                fontWeight: filtroTipo===f.val?700:500, border:'1.5px solid transparent',
                fontFamily:'inherit',
                background: filtroTipo===f.val?f.bg:'#f3f4f6',
                color:      filtroTipo===f.val?'#fff':'var(--text-secondary)',
              }}>{f.label}</button>
            ))}
          </Card>
          ...
          <Card style={{ padding:0 }}>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr>{['Fecha','Alumno','Grupo','Tipo','Gravedad','Descripción','Generado por'].map(h=><th key={h} style={thS}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {reportesGlobales.map(r=>{
                    ...
                    return (
                      <tr key={r.id}
                        ...
                        onClick={()=>seleccionarAlumno(ALUMNOS_MOCK.find(a=>a.nombre===r.alumno)||{id:r.id,nombre:r.alumno,grupo:r.grupo,matricula:'—',puntos:r.ptsDespues})}
                        ...
                        <td style={tdS}><span style={{ background:ts.bg, color:ts.color, fontSize:12, fontWeight:700, padding:'3px 10px', borderRadius:20 }}>{r.tipo}</span></td>
                        <td style={{ ...tdS, fontSize:12, color:'var(--text-secondary)' }}>{GRAVEDAD_LABEL[r.gravedad]}</td>
                        <td style={{ ...tdS, maxWidth:200 }}>{r.desc}</td>
            ...
            <div style={{ padding:'10px 14px', fontSize:13, color:'var(--text-secondary)', borderTop:'1px solid var(--border)' }}>
              {reportesGlobales.length} reportes · Haz clic en una fila para ver el historial completo del alumno
            </div>
          </Card>
        </>
      )}

      {/* ── Vista historial de alumno ── */}
      {!vistaGlobal && alumno && (
        <>
          <div style={{ background:'linear-gradient(135deg,var(--green-800) 0%,var(--green-600) 100%)', borderRadius:'var(--radius-lg)', padding:'1.25rem 1.5rem', color:'#fff', marginBottom:'1.25rem' }}>
            ...
            <div style={{ fontSize:14, opacity:0.85, marginTop:2 }}>{alumno.nombre} · {alumno.grupo}</div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
            {reportesFiltrados.length===0 ? (
              <Card style={{ padding:'2rem', textAlign:'center', color:'var(--text-secondary)' }}>No hay reportes de tipo "{filtro}" para este alumno.</Card>
      ...
      {!vistaGlobal && !alumno && (
        <Card style={{ padding:'3rem', textAlign:'center', color:'var(--text-secondary)' }}>
          ...
          <p>Busca un alumno para ver su historial de reportes.</p>
        </Card>
      )}
    </div>
  );
