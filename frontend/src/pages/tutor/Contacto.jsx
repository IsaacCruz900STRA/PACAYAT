...
const DIRECTIVOS = [
  { nombre:'Dr. Roberto Sánchez Díaz', rol:'Director',              ext:'101', color:'#dcfce7', textColor:'#14532d' },
  { nombre:'Mtro. Carlos Ramírez',     rol:'Subdirector Académico', ext:'103', color:'#dbeafe', textColor:'#1e40af' },
  { nombre:'Laura Torres Méndez',      rol:'Secretaria Escolar',    ext:'102', color:'#fce7f3', textColor:'#9d174d' },
];

const INFO_CARDS = [
  {
    icon:'📞', iconBg:'#dbeafe', title:'Teléfono', sub:'Línea principal',
    main:{ text:'951 123 4567', color:'#2563eb', size:24, bold:true },
    extra:['Extensión 101 - Dirección','Extensión 102 - Secretaría'],
  },
  {
    icon:'✉️', iconBg:'#dcfce7', title:'Correo Electrónico', sub:'Email institucional',
    main:{ text:'secundaria177@oaxaca.edu.mx', color:'#16a34a', size:15, bold:true },
    extra:['Tiempo de respuesta: 24-48 horas hábiles'],
  },
  {
    icon:'📍', iconBg:'#f3e8ff', title:'Dirección', sub:'Ubicación física',
    main:null,
    extra:['Av. Independencia #123','Col. Centro, C.P. 68000','Oaxaca de Juárez, Oaxaca'],
  },
  {
    icon:'🕐', iconBg:'#ffedd5', title:'Horario de Atención', sub:'Lunes a Viernes',
    main:null,
    horario:[
      { area:'Dirección:', hrs:'8:00 AM - 2:00 PM' },
      { area:'Secretaría:', hrs:'7:00 AM - 3:00 PM' },
      { area:'Prefectura:', hrs:'7:00 AM - 3:00 PM' },
    ],
  },
];
  ...
  return (
    <div style={{ padding:'0 2rem 2rem' }}>
      ...
      <div style={{
        ...
        borderRadius:'var(--radius-lg)', padding:'1.75rem 2rem', color:'#fff', marginBottom:'1.5rem', marginTop:'1.5rem',
      }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1.5rem' }}>
        {INFO_CARDS.map(card => (
          <Card key={card.title} style={{ padding:'1.5rem' }}>
            ...
            {card.main && (
              <div style={{ fontSize:card.main.size, fontWeight: card.main.bold ? 700 : 400, color:card.main.color, marginBottom:8 }}>
                {card.main.text}
              </div>
            )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem' }}>
          {DIRECTIVOS.map(d => (
            <div key={d.nombre} style={{ textAlign:'center' }}>
              <div style={{
                width:70, height:70, borderRadius:'50%', background:d.color,
              ...
              <div style={{ fontSize:13, color:d.textColor, fontWeight:600, marginTop:4 }}>Ext. {d.ext}</div>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:'var(--radius-lg)', padding:'1.25rem 1.5rem' }}>
        ...
        {['Llamando al teléfono principal y solicitando tu cita','Enviando un correo electrónico con tu solicitud','Acudiendo personalmente a la secretaría escolar'].map(item => (
          <div key={item} style={{ display:'flex', gap:8, alignItems:'flex-start', fontSize:14, color:'#1e40af', marginBottom:6 }}>
