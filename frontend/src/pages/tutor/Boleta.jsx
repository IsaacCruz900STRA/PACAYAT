...
import { HIJOS_MOCK, PERIODOS_MOCK, MATERIAS_MOCK } from './_mockData';
...
export default function TutorBoleta() {
  ...
  const promedioGeneral = (() => {
    const proms = MATERIAS_MOCK.map(m => promedio(m.cals)).filter(v => v !== null);
    if (!proms.length) return null;
    return (proms.reduce((a,b)=>a+b,0)/proms.length).toFixed(1)*1;
  })();
  ...
  const tdS = { padding:'11px 14px', borderBottom:'1px solid var(--border)', fontSize:14, verticalAlign:'middle' };
    ...
    <div style={{ padding:'0 2rem 2rem' }}>
      ...
      {!loadingHijos && hijoActual && (
        <>
          ...
          {/* Promedio general */}
          <Card style={{ border:`2px solid ${calColor(promedioGeneral)}22`, marginBottom:'1.25rem', padding:'1.5rem', textAlign:'center' }}>
          ...
          {/* Tabla calificaciones por materia */}
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr>
                    ...
                    {PERIODOS_MOCK.map(p => <th key={p} style={{ ...thS, minWidth:90 }}>{p}</th>)}
                        ...
                        {m.cals.map((v,i) => (
                          <td key={i} style={{ ...tdS, textAlign:'center' }}>
                            {v !== null
                              ...
                              : <span style={{ color:'var(--text-muted)', fontSize:12 }}>Pendiente</span>}
                          </td>
                        ))}
