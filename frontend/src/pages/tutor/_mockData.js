...
// Datos de prueba — reemplazar con useFetch() cuando el backend esté listo.
...
// Calificaciones por materia (5 periodos, null = pendiente)
export const MATERIAS_MOCK = [
  ...
  { materia:'Formación Cívica', cals:[8.5, 8.2, 8.0, 8.0, null], faltas:[0,0,0,0,0] },
...
export const REPORTES_MOCK = [
  { tipo:'Negativo', gravedad:'MEDIO',       delta:-5,  fecha:'26 de abril, 2026',  hora:'2:15 PM',  desc:'Llegó tarde a clase de matemáticas sin justificación. Se habló con el alumno sobre la importancia de la puntualidad.', por:'Luis Ramírez Santos (Prefecto)', ptsAntes:63, ptsDespues:58 },
...
// Horario semanal (mismo para todos — en producción vendrá del backend por alumno)
...
export function colorMat(mat) {
  let h = 0;
  for (const c of mat) h = (h * 31 + c.charCodeAt(0)) % COLORES_MAT.length;
  return COLORES_MAT[h];
}
