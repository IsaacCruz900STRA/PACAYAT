...
import { Search, X, User, ClipboardList, ChevronDown, Calendar } from 'lucide-react';
...
export default function PrefectoReportes() {
  ...
  // Cargar reportes del alumno seleccionado
  ...
  // Llegó desde Alumnos con query params
  useEffect(() => {
    const id  = Number(searchParams.get('alumno'));
    const nom = searchParams.get('nombre');
    if (id && nom) {
      const preset = { id, nombreCompleto: nom, nombre: nom };
      setAlumno(preset);
      setQuery(nom);
      cargarReportes(id);
    }
              ...
              {!loadingSearch && resultados.map(a => {
                ...
                return (
                  <div key={a.id} onClick={() => seleccionar(a)}
                    ...
                    <span style={{ background: st.bg, color: st.color, fontSize: 12, fontWeight: 700, padding: '2px 10px', borderRadius: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Calendar size={13} /> {formatDateTime(r.creadoEn)}
                        </div>
